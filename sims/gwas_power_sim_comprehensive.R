################################################################################
#   GWAS POWER SIMULATION (V7): FOUR-METHOD COMPARISON
################################################################################
...
#    2. Mixture Model: Adaptive, "soft" classification based on MFI distribution.
#    3. Per-Sample Valley Cutoff: A "naive" hard cutoff found from the valley
#       of the density plot of each individual sample.
#    4. Noisy External Cutoff: A realistic hard cutoff derived from a small,
#       separate, imperfect calibration panel.
################################################################################
...
raw_results <- future_map_dfr(1:nrow(full_sim_grid), ~{
    ...
    sim_data <- sim_data %>%
        mutate(
            # FIXED: Replaced faulty if_else() with a standard if-else block logic
