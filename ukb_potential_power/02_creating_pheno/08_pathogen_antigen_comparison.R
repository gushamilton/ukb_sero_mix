...
for (pth in names(pathogen_map)) {
  ...
  gwas_outputs[[pth]] <- pathogen_data %>%
    ...
    mutate(
      ...
      # Weights for GWAS (using soft probability as weight)
      !!glue("{tolower(pth)}_weight") := pathogen_data$bayes_soft
    )
...
for (pth in names(gwas_outputs)) {
  ...
  # Weight files
  write_tsv(res %>% select(FID, IID, Weight = !!glue("{low}_weight")),
            glue("quickdraws_input/{low}_weights.weights"))
}
...
summary_table <- map_dfr(names(gwas_outputs), function(pth) {
  ...
  tibble(
    ...
    Mean_Weight = mean(res[[glue("{low}_weight")]], na.rm = TRUE)
  )
})
...
cat("- Weight files for GWAS analysis\n")
