...
# After master_pheno_table is loaded, load covariate table to get PC1_assay_noise
if (file.exists("covariates_master.tsv")) {
  covar_table <- read_tsv("covariates_master.tsv", show_col_types = FALSE) %>%
    select(IID, PC1_assay_noise)
} else if (file.exists("covariates_master.tsv")) {
  covar_table <- read_tsv("covariates_master.tsv", show_col_types = FALSE) %>%
    select(IID, PC1_assay_noise)
} else {
  warning("Covariate file with PC1_assay_noise not found; PC1 plots will be skipped.")
  covar_table <- NULL
}
...
plot_original_distribution <- function(antigen_name, raw_data, threshold, model_fits = NULL) {
  ...
  if (!is.null(model_fits) && antigen_name %in% names(model_fits)) {
    ...
    density_pos <- sn::dst(x_points, xi = fit$mu[pos_comp], omega = sqrt(fit$sigma2[pos_comp]), 
                          alpha = fit$shape[pos_comp], nu = fit$nu[pos_comp]) * fit$pii[pos_comp]
    density_neg <- sn::dst(x_points, xi = fit$mu[neg_comp], omega = sqrt(fit$sigma2[neg_comp]), 
                          alpha = fit$shape[neg_comp], nu = fit$nu[neg_comp]) * fit$pii[neg_comp]
    ...
    p <- p +
      ...
      geom_line(data = density_df, aes(x = x, y = total), color = "black", linewidth = 1.5)
  }
...
plot_pc1_vs_igg <- function(antigen_name, pheno_data, covar_data) {
  if (is.null(covar_data)) {
    return(ggplot() + annotate("text", x = 0.5, y = 0.5, label = "PC1 data not available") + theme_void())
  }
  igg_col  <- glue("{antigen_name}_IgG_raw")
  soft_col <- glue("{antigen_name}_sero_soft")
...
plot_pc1_trends <- function(antigen_name, pheno_data, covar_data) {
  ...
  if (!all(c("IID", soft_col, hard_col) %in% names(pheno_data))) {
    return(ggplot() + annotate("text", x = 0.5, y = 0.5, label = "Required columns missing for this antigen") + theme_void())
  }
  
  merged <- pheno_data %>%
    select(IID, soft = all_of(soft_col), hard = all_of(hard_col)) %>%
    left_join(covar_data, by = "IID") %>%
    filter(!is.na(soft) & !is.na(hard) & !is.na(PC1_assay_noise))
  ...
  # Add IgG data and create four-facet plot
  igg_col <- glue("{antigen_name}_IgG_raw")
  if (igg_col %in% names(pheno_data)) {
    merged <- merged %>%
      left_join(pheno_data %>% select(IID, igg = all_of(igg_col)), by = "IID") %>%
      filter(!is.na(igg))
    ...
    return(ggplot() + annotate("text", x = 0.5, y = 0.5, label = "IgG data not available") + theme_void())
  }
}
